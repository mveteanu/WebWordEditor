<%
' *********************************************************************
' Control grafic de tipul THTMLEditor realizat prin generarea pe server
' in ASP si folosind script client VBScript (incapsulat in HTC) pentru 
' interactiune in Browser
' Autor: Marian Veteanu
' Data:  02 aprilie 2001
' *********************************************************************
...
' Subrutina principala care deschide controlul si initializeaza 
' unele variabile server
' Intrare:
...
'   iWidth = lungimea controlului
'   iHeight = inaltimea controlului
'   HTCName = calea catre fisierul .htc ce contine partea activa de client
'   ImagesDir = calea catre directorul ce contine imaginile folosite la crearea toolbar-urilor
...
' Incepe o noua pagina de editare in cadrul controlului
' strWaterMark daca este diferit de "" atunci el indica calea
' catre o imagine ce va fi afisata pe post de watermark pe pagina
...
' Declaratii private de variabile
...
' Incepe declararea toolbarurilor
' In pagina .asp scrisa de utilizator aceste apeluri trebuie
' sa fie dupa apelul principal de deschidere al controlului

' Defineste toolbarul cu comenzi comune de formatare a textului
...
' Defineste toolbarul cu comenzi pentru inserarea si manipularea imaginilor
...
' Defineste toolbarul cu comanda de Zoom
...
' Creaza minifereastra care cere numele imaginii precum si formul
' ascuns cu elemente de tip: INPUT file
...
' Intoarce textul problemei stocat in BD
Function GetDOCText(docid, objCon)
 Const SQLQ = "SELECT * FROM TBDocs WHERE id_doc=@1"
 Dim rs, re
 Set rs = objCon.Execute(Replace(SQLQ, "@1", CStr(docid)))
 If not rs.EOF then re = rs.Fields("text").Value Else re = ""
 rs.Close
 set rs = nothing
 GetDOCText = re
End Function
